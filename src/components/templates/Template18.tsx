...
export default function Template18({ data }: Template18Props) {
  ...
  return (
    ...
    >
      {isVideo ? (
        <video ref={videoRef} src={mediaUrl} className="absolute inset-0 w-full h-full object-cover filter blur-2xl scale-125 opacity-20" muted loop autoPlay playsInline />
      ) : (
