    ...
    >
        ...
        <div className="w-full max-w-sm h-full flex flex-col items-center justify-center">
            ...
            <div className="mt-4 w-full max-w-[200px] flex-shrink-0">
                <p className="text-xs text-slate-800 mb-2">you are my favorite song</p>
                <div className="relative w-full h-0.5 bg-slate-800">
                    <div 
                        className="absolute top-[-3px] w-2 h-2 rounded-full bg-slate-800"
                        style={{ left: `${progress}%`}}
                    ></div>
                </div>
                <div className="flex justify-center gap-5 mt-2.5">
                    <div className="h-10 flex items-center">
                        <button onClick={(e) => { e.stopPropagation(); seek(-5); }} className="w-0 h-0 border-t-[10px] border-t-transparent border-b-[10px] border-b-transparent border-r-[20px] border-r-slate-800" aria-label="Rewind"></button>
                    </div>
                    <button onClick={handlePlayPause} className="w-10 h-10 rounded-full bg-slate-800 flex justify-center items-center text-white" aria-label="Play/Pause">
                        {isPlaying ? <Pause size={20} /> : <Play size={20} className="ml-1" />}
                    </button>
                    <div className="h-10 flex items-center">
                        <button onClick={(e) => { e.stopPropagation(); seek(5); }} className="w-0 h-0 border-t-[10px] border-t-transparent border-b-[10px] border-b-transparent border-l-[20px] border-l-slate-800" aria-label="Fast Forward"></button>
                    </div>
                </div>
            </div>
        </div>
